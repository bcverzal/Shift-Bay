...
function run() {
  ...
  includes(index, 'id="planSlotDate" type="date"', "training-plan availability must use a calendar date input");
  includes(index, 'id="planSlotStart" data-time-picker', "training-plan availability must use the shared start-time picker");
  ...
  excludes(app, "selected.length > totalNeeded || score > bestScore", "training-plan optimization must not use the empty plan's score to prune all candidates");
  includes(app, "function trainingCandidateDiagnostics", "empty training proposals must explain which candidate filter excluded the schedule");
  includes(app, "function trainingProposalTimingBounds", "training proposals must constrain manual time edits to the shared availability");
  includes(app, "Trainer scheduled", "training proposals must show the trainer's existing scheduled shift");
  includes(app, "data-edit-training-proposal", "training proposal cards must expose the familiar double-click editing interaction");
  includes(app, "function openTrainingProposalEditor", "double-clicking a proposal must open the pending shift in the shift editor");
  includes(app, "function proposalOverlapsTraineeSchedule", "replacement training proposals must avoid an accepted trainee shift");
  includes(app, "selected.some((chosen) => trainingRangesOverlap(chosen, candidate))", "a proposed training plan must not contain overlapping trainee shifts");
  includes(app, "function acceptedTrainingPlanBay", "accepted training shifts must remain visible while replacements are reviewed");
  includes(app, "function resetEmployeeTrainingPlan", "managers need a deliberate way to remove plan-generated training shifts for a fresh plan");
  includes(app, "Training plan complete", "a fully accepted training plan must show a completion summary");
  includes(app, "isTrainerShift: true", "accepting training must mark the trainer's existing shift as a training assignment");
  includes(app, "trainerSourceShiftId", "the trainee shift must retain a link to the trainer's source shift");
  includes(app, "function restoreTrainerSourceForTraineeShift", "removing or resetting a training plan must restore the trainer's original shift");
  includes(app, "function trainingAssignmentIsValid", "a paired training shift must verify its trainer source before remaining assigned");
  includes(app, "function reconcileTrainingAssignments", "manual schedule changes must reconcile invalid training pairs");
  includes(app, "function clearTrainingAssignment", "an invalid training pair must clear the trainee's stale trainer assignment");
  includes(app, "function attachUnambiguousTrainerSource", "manual training shifts must only retain a trainer when one scheduled source shift is unambiguous");
  includes(app, "const clearedTrainingAssignments = reconcileTrainingAssignments()", "dragging or editing a shift must immediately revalidate its training assignment");
  includes(app, "trainerSourceShiftId: preservesExistingPair", "ordinary shift edits must retain a valid accepted training pair until it is revalidated");
  assert.match(app, /function renderAll\(options = \{\}\) \{[\s\S]*?reconcileTrainingAssignments\(\);[\s\S]*?normalizeSavedEmployeePhones\(\);/, "full renders must repair orphaned trainer markers after manual schedule changes");
  includes(app, "function prepareMovedTrainingShift", "moving a training shift must resolve its trainer before committing the move");
  includes(app, "function trainerSourcesForMovedTrainingShift", "training moves must search eligible scheduled trainer sources on the target date");
  includes(app, "function pairTrainingShiftWithTrainerSource", "a selected trainer source must be linked atomically to the moved trainee shift");
  includes(app, "Training Shift Cannot Move", "a move without a qualified scheduled trainer must be denied");
  includes(app, "Choose a Trainer", "a training move with viable candidates must let the manager select the trainer");
  ...
  excludes(index, 'id="shiftTrainingDayOverride"', "training-day numbers must not be manually fixed and duplicated");
  ...
  includes(app, "trainingMealStartTimes", "training settings must retain configurable preferred meal training start times");
  includes(index, 'id="exportTrainingSetupBtn"', "training setup must be exportable without exporting a schedule backup");
  includes(index, 'id="trainingSetupImportInput"', "training setup imports must use a dedicated control");
  includes(app, "function trainingSetupExportPayload", "training setup exports must use a narrow payload");
  includes(app, "function prepareTrainingSetupImport", "training setup imports must validate a narrow payload");
  includes(app, "TRAINING_SETUP_TRANSFER_APP", "training setup imports must identify their own file type");
  includes(app, "roleMealTraining", "training setup transfer must preserve meal qualifications");
  includes(app, "trainerPriorityIds", "training setup transfer must preserve trainer priorities");
  includes(app, "Training setup import tried to change schedule or availability data.", "training setup imports must guard schedule and availability data");
  includes(app, "no employee will be created or matched by name", "training setup imports must skip unmatched local employees instead of guessing");
  ...
  includes(app, 'document.body.classList.add("printing-floor-plan")', "single floor-plan printing must target the floor plan regardless of the active tab");
  includes(styles, "body.printing-grid #schedule.panel", "schedule grid printing must not depend on the Schedule tab being active");
  excludes(styles, "body.printing-grid #schedule.panel.active", "schedule grid printing must not be coupled to active screen state");
  includes(app, 'const dayViewToRestore = layout === "grid" ? focusedDateKey : ""', "weekly grid printing must temporarily render the week when Day View is active");
  includes(styles, "body.printing-floor-plan .floor-plan-sheet", "single floor plans must retain their print dimensions outside the active Floor Plans tab");
  includes(app, 'Continue Training Plan', "employee training progress must distinguish resumable training plans");
  includes(app, 'Choose an intended role to begin training', "employees without a role must be shown the next training setup action");
  includes(app, 'data-open-training-roles', "employees without a role must be able to open the Roles tab from training");
  includes(app, 'draggable="true"', "trainer priority rows must support drag-and-drop ordering");
  includes(app, 'function syncTrainingPriorityRows', "trainer priority cards must retain a normalized saved order after reordering");
  includes(index, "Keep Accepted for Later", "training plans must allow managers to continue later with accepted shifts retained");
  includes(app, "training-plan-proposal-card", "training proposals must render as schedule-like cards");
