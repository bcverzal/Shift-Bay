...
const productionAtomicSql = fs.readFileSync(path.join(root, "supabase", "production-atomic-snapshot-write.sql"), "utf8");
...
assert.match(productionAtomicSql, /create or replace function public\.write_production_schedule_atomically_with_snapshot/i);
assert.match(productionAtomicSql, /security definer/i);
assert.match(productionAtomicSql, /write_normalized_schedule_atomically/i);
assert.match(productionAtomicSql, /insert into public\.scheduler_state_documents/i);
assert.match(productionAtomicSql, /on conflict \(location_id, document_key\) do update/i);
...
assert.match(edge, /function normalizedProductionAtomicWritesEnabled\(\)/);
assert.match(edge, /SHIFT_BAY_PRODUCTION_ATOMIC_WRITES/);
...
assert.match(edge, /saveMode === "normalized-production-atomic-revision"/);
assert.match(edge, /Production atomic schedule writes are disabled until the controlled cutover/);
assert.match(edge, /Production atomic schedule writes are limited to the configured primary location/);
assert.match(edge, /function schedulePopulationSummary\(/);
assert.match(edge, /function destructiveScheduleWriteReason\(/);
assert.match(edge, /before\.employees >= 20/);
assert.match(edge, /before\.scheduleItems >= 100/);
assert.match(edge, /scheduler_state_destructive_write_blocked/);
assert.match(edge, /"state,saved_at,updated_at"/);
...
assert.match(edge, /supabaseJsonAll\(`\/shifts\?location_id=/);
assert.match(edge, /normalizedOpenShifts\.length !== snapshotOpenShifts\.length/);
assert.match(edge, /snapshotOpenShifts\.some\(/);
assert.match(edge, /unassignedShifts: normalizedOpenShiftFallback \? snapshotOpenShifts : normalizedOpenShifts/);
