...
const CURRENT_READ_SOURCE = LEGACY_SNAPSHOT_OVERRIDE
  ? "legacy-snapshot"
  : NORMALIZED_SCHEDULE_PRODUCTION_ATOMIC_MODE
    ? "normalized-production-atomic-revision"
...
function serverEnvelope(options = {}) {
  ...
  return {
    ...
    saveMode: NORMALIZED_SCHEDULE_DIRECT_WRITE_MODE && options.scope !== "employee-profile"
      ...
      : "snapshot-bridge",
    normalizedScheduleRevision: (NORMALIZED_SCHEDULE_REVISION_CANARY_MODE || NORMALIZED_SCHEDULE_ATOMIC_CANARY_MODE || NORMALIZED_SCHEDULE_PRODUCTION_ATOMIC_MODE)
...
function floorPlanEmployeeLine(employee, shift, firstNameCounts = {}, context = {}) {
  ...
  const menuTestNote = trainingTestForShift(shift) ? `<span class="floor-menu-test-note">Menu Test</span>` : "";
  ...
  return `<span class="${nameClass}">${floorPlanEmployeeName(employee, firstNameCounts)}</span><span class="floor-time">${timeText}</span>${trainingNote ? `<span class="floor-training-note">${trainingNote}</span>` : ""}${menuTestNote}${extraNote ? `<span class="floor-note-extra">${extraNote}</span>` : ""}`;
}
...
function renderFloorPlanSheetMarkup(dateKey, period, options = {}) {
  ...
  const overlay = (className, values) => {
    const list = Array.isArray(values) ? values : [values];
    const entries = list.filter(Boolean).map((value) => className === "floor-servers" ? value : `<div>${value}</div>`).join("");
    return `<div class="floor-overlay ${className}">${entries}</div>`;
  };
  return `
